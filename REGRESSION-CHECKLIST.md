...
- [ ] **B11** **No hero/chart-flipping toggle.** The centre hero always reads whole-centre and the chart lines are fixed (neon = your shops, sage dashed = whole centre); no control flips which dataset is the neon line. The brand-grid My/All radio (B14a) is the only mode control and is scoped to the brand list. The choice **persists across reload**.
...
- [ ] **B14a** **My brands / All brands radio (restored Jun 2026)** sits above the brand grid for followers (`showBrandFilterTabs`), showing `My brands {N}` / `All brands {M}` counts. Clicking switches the grid (and its heading) between followed shops and all shops at the centre, **persists across reload** (`tide_view_mode`), and **preserves scroll position** (no jump to top). It does **not** change the hero verdict or the chart lines. Logged-out users / users who follow no brands see no radio and the full grid.
