...
`/api/rescore.js` is a Vercel serverless function that wraps `runScoring()` from `score.js`. The admin panel (`admin.html`) fires a fire-and-forget POST to it after every successful `applyAction` / `applyEdit`, so the public site reflects admin edits within a couple of seconds without waiting for the 10:00 UTC cron.

**Admin writes are atomic.** Each verb (confirm start/end/on/off, edit, delete) goes through a `SECURITY DEFINER` RPC (`admin_confirm_start` / `admin_confirm_end` / `admin_confirm_status` / `admin_edit_cycle` / `admin_delete_cycle` — migration `20260622_admin_write_rpcs.sql`) that does the cycle write + event write + review-log in one transaction and re-checks `is_admin()` itself. This removes the old half-commit class (orphaned open cycle, `active_cycle_id` left pointing at a closed/deleted cycle). The client **falls back to the legacy multi-step PostgREST path** when the RPC is absent (detected via `isRpcMissing`), so deploying the static site before applying the migration doesn't break the console. Carry-forward in `score.js` (if no brand at the centre has `last_verified_date == today`, reuse yesterday's brand counts) keeps the rescore cheap and lets the state machine re-derive (e.g. a Peak rolls to Easing) without forcing the admin to re-verify a brand they already know is on sale, and preserves narratives on untouched centres.
