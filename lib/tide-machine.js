...
export const TIDE = {
  ...
  CREST_DROP_BAND: 4.0,   // FLAT score this far below the recent crest exits Peak (D19)
  CREST_WINDOW_DAYS: 14,  // window the recent crest is measured over (D19)
...
export function trajectoryStep(score, recentScores, priorTrajectory, observedDays, C = TIDE) {
  ...
  if (prior === 'RISING') {
    ...
    // Sustained-decline decay (D20): a steady slow slide of ~0.5–0.7 pt/day
    // slips between the FLAT band (per-step drop too small) and the stall
    // check (3-day span too wide), so it read a sticky RISING all the way
    // down and the FLAT-only crest release never fired — pinning "Go now" on
    // visibly dropping centres. Two consecutive down days at a fresh low is
    // not a climb → FLAT. A one-day dip off a plateau still holds RISING (the
    // one-day grace). RISING-only, stateless — parity with getTrajectory.
    if (score < recentScores[0] && recentScores[0] < recentScores[1]) return 'FLAT';
    return 'RISING';
  }
  if (prior === 'FALLING') {
...
export function stageStep(score, prevStage, traj, prevTraj, recentCrest = null, C = TIDE) {
  ...
  // Crest-distance release (D19): FLAT this far below the recent crest is a
  // stalled/frozen decline off the far side, not a plateau at the top — exit
  // Peak. FLAT-only; null crest → never fires (behaviour-preserving default).
  // The D20 sustained-decline decay in trajectoryStep is what makes FLAT-only
  // sufficient — a slow slide now decays to FLAT instead of a sticky RISING.
  const stalledDecline = traj === 'FLAT'
    && Number.isFinite(recentCrest) && score <= recentCrest - C.CREST_DROP_BAND;
  ...
  const crestHold = wasHigh && traj !== 'FALLING' && !stalledDecline;
...
export function initTideState(observation, C = TIDE) {
  ...
  const { stage, verdict } = stageStep(score, null, trajectory, null, null, C);
  return {
...
export function nextTideState(prevState, observation, C = TIDE) {
  ...
  // Recent-crest buffer for the D19 stalled-decline release. Additive state,
  // date-windowed to CREST_WINDOW_DAYS and cleared on a broken window (mirrors
  // the trajectory reset — a post-gap centre can't exit Peak on a pre-gap
  // crest). Excludes today (built from prevState + prior buffer), matching
  // score.js's `.lt('score_date', TODAY)` crest source.
  const crestRecent = windowBroken
    ? []
    : [{ date: prevState.date, score: prevState.score }, ...(prevState.crestScores || [])]
        .filter(e => (daysBetween(e.date, date) ?? Infinity) <= C.CREST_WINDOW_DAYS);
  const recentCrest = crestRecent.length ? Math.max(...crestRecent.map(e => e.score)) : null;

  const observedDays = prevState.observedDays + 1;
  const trajectory = trajectoryStep(score, recent, priorTraj, observedDays, C);
  const { stage, verdict } = stageStep(score, prevState.stage, trajectory, priorTraj, recentCrest, C);

  const stageEnteredDate = stage === prevState.stage ? prevState.stageEnteredDate : date;
  return {
    date, score, stage, verdict, trajectory,
    recentScores: recent,
    crestScores: crestRecent,
    observedDays,
    stageEnteredDate,
...
export function stateFromRow(row, priorRowCount = 3) {
  ...
  return {
    ...
    crestScores: Array.isArray(row.crestScores) ? row.crestScores : [],
