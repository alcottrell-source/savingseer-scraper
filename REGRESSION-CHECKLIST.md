...
- [ ] **A8** Centre detail view: shows centre name + city, **merged tide card** (large Tide Score **%** + verdict word, a **per-line weekly movement figure under each %** ("▲ 12 (8%) vs last week" — chevron + shop-count change + percentage-point change in brackets, neon up / coral down / muted "Level", vs the stored point ~7 days ago; omitted when <~3 days of history), "Verified {when}" badge, saved-shops line, "N of M shops on sale" **state-only** count line, a quiet chart **key** (swatch + label, only when both the your-shops + whole-centre lines are present), faithful 0–100 history chart with 7D/30D/60D/MAX tabs, **date-based**, defaulting to 60D, with y-axis numbers in a right-hand gutter, narrative, brand grid, "Back to Centres" link. There is no longer a separate "Tide over 60 days" card below the vessel — the chart is inside the same card as the verdict.
...
- [ ] **A11** Merged tide card consistency: the centre-detail hero shows a single verdict word (beside the %); the trend-pill and chart-eyebrow tail word were retired (direction now lives in the per-line weekly movement figures under each %). The big "%" (from `tide_score`), the "N of M shops" count (from `brands_on_sale`/`total_brands`), and the chart's endpoint pill all read the same value — they cannot contradict because the % is computed from the count. "Go now" copy appears only on PEAK.
...
- [ ] **B11** **Selector lives on the brand list (restored Jul 2026).** A `[ Your shops (N) | All shops (M) ]` segmented pill (`.brand-filter-tabs`) sits **directly under the "Your shops here" brand-list heading** and flips `brandFilter`. It appears **only** on a centre where you follow ≥1 shop present (`showBrandFilterTabs`); anon/no-follow users see no control (All everywhere). The choice **persists across reload** and re-personalises the landing "Today's tide" (shared global `tide_view_mode`). The **centre hero is NOT affected** — its two-column metric rows and fixed neon/dashed chart encoding show both lenses at once regardless of the selector.
...
- [ ] **D9** Centre with sparse history (< 5 real days): merged card's history chart renders **faithfully** (real `tide_history` points only — no synthetic UK-retail-calendar backfill, no amplitude rescaling); today's point is anchored to the live headline value and sits at the right edge; the per-line "vs last week" movement figure is omitted when there's <~3 days of history to compare; no NaN coordinate and no console error. (The synthetic seasonal backfill remains only on the out-of-scope all-centres landing chart.)
