...
- `brand_sale_cycle_changes` (Jun 2026, anon read enabled) — intra-cycle audit log of discount-%/sale-type changes. The **same** open cycle staying put while its terms move (e.g. 50%→60%, or `% off`→`flash`) is the same sale deepening/shifting, not a new one — `brand_sale_cycles.max_discount_pct`/`sale_type` hold the *current* value; this table is the history behind them. Written by `recordCycleChange` in `admin.html` from both edit paths (`applyEdit` and the `confirm_start` reuse-an-open-cycle branch) — one row per edit that actually moves the %/type (no-op or start-date-only edits log nothing); only the side that moved is filled. `ON DELETE CASCADE` from the cycle. Surfaced as a "Changes" trail on the admin card and a "Changed during this sale" trail per episode in the public shop detail sheet (joined onto `BRAND_CYCLES[name][i].changes` by cycle id). Does **not** affect the Tide Score (score is brand-on-sale *count*, not depth).
