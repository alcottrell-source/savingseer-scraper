...
**Your-shops / Whole-centre toggle (Jun 2026 revision).** The hero's view mode is switched by a **segmented toggle** `[ Your shops | Whole centre ]` (`.tide-mode-toggle`/`.tide-mode-seg`, calling `setTideMode`), sitting between the count line and the chart where a quieter inline text link used to be. The **selected view drives three things at once**: (1) the big headline `%` — `personalPct` + a muted "on sale" label in Your-shops mode (no verdict/recommendation word — recommendation language is centre-PEAK-only per the verdict rules), or the centre `%` + verdict word in Whole-centre mode; (2) the single count line — "N of M shops you follow on sale · {named shops}" (the named shops folded in from the **retired star row** — `renderSavedShopsLine` now renders only the follow-CTA for non-followers) vs the centre density + trajectory clause; (3) **which chart line is the neon-green hero** — the selected dataset is painted neon (solid, glow, endpoint pill) and the other drops to the cyan **dashed secondary**, with the legend relabelling to match (`buildTideChartSVG` takes the primary as `series`, the secondary as `personal`, and labels via `opts.primaryLabel`/`opts.secondaryLabel`). The active toggle segment is neon-green to reinforce "selected view = the green line". The toggle only appears when the user follows ≥1 brand present at this centre (`canToggle`); otherwise the hero locks to the centre view with no toggle. An earlier cryptic "My/All" segmented control tested poorly ("not clear enough") and was briefly a one-tap inline disclosure (`Showing your shops · see the whole centre`) — the toggle is back with **explicit labels** and a clear selected state, and now governs the headline number and hero line, not just the brand-chip grid. Because Your-shops mode leads with the personal `%` + "on sale" (no verdict word), a compact **whole-centre context chip** ("Whole centre **59%** · {verdict}", `.tide-context-chip`) sits under the count line in that mode so the centre's verdict — especially an actionable PEAK **"Go now"** (gold, `.cc-verdict.tone-go`) — stays visible without a big duplicate number. This is the only verdict word shown in Your-shops mode and is explicitly attributed to the centre, so it doesn't violate the single-verdict-word rule (which guards against two labels for the *same* thing, not against labelling the centre while the personal % leads).
