...
- The centre-detail hero shows a **single** verdict word (beside the `%`). The trend pill + chart-corner badge were retired (Jun 2026); direction is carried by the **per-line weekly movement figure** under each headline % (`tideWeekMoveHTML`) — today's live value vs the stored point ~7 days ago, shop-count change + percentage-point change in brackets, chevron colour-cued neon-up/coral-down, omitted when there's too little history. The count line is **state only**. Movement appears once per line and nowhere else (no count-line clause, no legend badge) — the legend is a plain key.
