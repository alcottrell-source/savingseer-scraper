...
**Centre-detail hero rebuild (Jun 2026).** The centre-detail vessel (`renderTideVessel`, now a single ctx object) leads with the **Tide Score as a large `%`** + the verdict word, a **"Verified {when}"** badge (most-recent `last_verified_date` across brands at the centre), a **saved-shops line** (`renderSavedShopsLine` — three states, mirrors the brand-sale alert copy), a **count line** ("N of M shops on sale, {direction} K two weeks ago" — count from `brands_on_sale`/`total_brands`, direction word from the engine `trajectory`, comparison count from the real `brands_on_sale` stored in `tide_history` two weeks ago, data-gated), and a **bold neon-green history chart** (`buildTideChartSVG` over `buildTideRealSeries`) with **7D/30D/MAX** tabs (`setTideChartPeriod`). The chart plots `tide_history` exactly as stored — **no synthetic backfill, no value rescaling, no monotonising** (the synthetic `buildTide60Series` path now serves only the all-centres landing chart). The flat-black panel + neon line is the hero; the **y-axis auto-zooms** to ~12% above the window's max (floored at PEAK+8 so PEAK is always on-chart, capped at 100) so the line fills the panel instead of leaving dead space up to 100 — this zooms the visible *axis window* only, never the plotted values, and small ceiling/0 scale labels keep it honest. Four reference lines (ceiling, PEAK, QUIET, 0); PEAK(40)/QUIET(15) come from `TIDE_PEAK_LINE`/`TIDE_QUIET_LINE`, a documented mirror of score.js's `HIGH_TIDE_ENTER`/`RISING_FLOOR`. Periods window by the last N real data **points** (not calendar days) so a centre with gappy history still draws a line on 7D. **The landing chart (`renderTide60Light`) mirrors the 7D/30D/MAX tabs** (`setTide60Period`, windowing the all-centres average) and is 50% taller, but otherwise keeps its original look (0–100 scale, 33/66 gridlines, date labels, trend pill). The old trend pill + chart-corner badge + statement subtitle were retired — direction lives in the count line now. **The hero count must equal `centre_seer_scores.brands_on_sale`** (it's the same source `tide_score` is computed from, so the `%` and the count can't contradict); it falls back to the local PRESENCE×SALE_STATUS count only when the server hasn't scored the centre yet.
