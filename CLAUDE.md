...
| Brand-sale alert | a followed brand is **on sale and not yet emailed** to the user | daily 07:00 UTC, `{}` | `brand_sale_alerts` (+ respects `excluded_brand_ids`) |
| Weekend digest | ≥1 saved centre at Rising or above | **Friday 19:00 UTC**, `{"digestOnly":true}` | `daily_digest` |

UI toggles in the account panel map to these three columns. Don't rename the columns — only the human-readable copy. The digest is **weekly (Friday)**, not daily — the email copy hard-codes "Friday" and that is now correct. Brand-sale alerts use a **combined, deduped roundup** (Jun 2026): each daily run gathers every currently-on-sale followed brand a user hasn't been emailed yet, sends **one email** listing them all (always the roundup layout, even for a single shop), and records each in the `brand_sale_notifications` table (PK `user_id` + `sale_key`, where `sale_key` = active cycle id or `nocycle:<brand_id>:<date_first_detected>`). This replaced the old "started today" one-shot — it combines sales across days, catches sales a stale run missed, and alerts users who follow an already-on-sale brand. A new cycle = a fresh key = a fresh alert. The table must exist before the function deploys (migration `20260625_add_brand_sale_notifications.sql`); the **first run after deploy sends a one-time catch-up email of all active sales** per user (pre-seed the table to suppress it). `digestOnly` runs pass 3 only; the default daily call runs passes 1+2 and skips the digest.
